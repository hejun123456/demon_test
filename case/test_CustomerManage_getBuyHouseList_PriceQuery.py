...
@ddt.ddt
class GetBuyHouseList_PriceQuery(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # 保持登录状态
    ...
    testdata = readexcel.ExcelUtil(CUSTOMER_MANAGE_EXCEL_PATH, sheetName="客源管理-求购-总价格查询").dict_data()
    @ddt.data(*testdata)
    def test_getBuyHouseList_PriceQuery(self, case):
        ...
        if len(buy_list) > 0:
            ...
            lst = []
            for i in buy_list:
                buy_price_high = i["housePriceHigh"]
                lst.append(buy_price_high)
                buy_price_low=i["housePriceLow"]
                lst.append(buy_price_low)
