...
@ddt.ddt
class GetLeaseHouseList_AreaQuery(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # 保持登录状态
        cls.s = requests.session()

    testdata = readexcel.ExcelUtil(HOUSE_MANAGE_EXCEL_PATH, sheetName="房源管理-出租-面积查询").dict_data()
    @ddt.data(*testdata)
    def test_getLeaseHouseList_AreaQuery(self, case):
        ...
        if len(lease_list) > 0:
            ...
            lst = []
            for i in lease_list:
                lease_area = i["leaseArea"]
                lst.append(lease_area)
            lst.sort()      #将出租房源面积从小到大排序
