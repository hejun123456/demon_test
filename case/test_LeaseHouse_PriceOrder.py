...
@ddt.ddt
class LeaseHousePriceOrder(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # 保持登录状态
        cls.s = requests.session()
        #创建出租中按照总租金来排序的房源
        cls.Lower_caseid=HouseManage.LeaseHouseOrder().create_LeaseHouse_LowerPrice()
        cls.High_caseid,cls.headers=HouseManage.LeaseHouseOrder().create_LeaseHouse_HighPrice()
        sleep(10)
    ...
    #出租房源按照总租金排序
    data = readexcel.ExcelUtil(HOUSE_MANAGE_EXCEL_PATH, sheetName="房源管理-出租-总租金排序").dict_data()
    print(data)
    @ddt.data(*data)
    def test_leaseHouse_total_AreaOrder(self,case):
        ...
        # 断言
        self.assertEqual(check.get("leaseTotalPrice"), res_text["data"]["leaseList"][0]["leaseTotalPrice"])
